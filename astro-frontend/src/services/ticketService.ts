import type { Ticket, TicketStatus, Priority } from '../models/Ticket';

// Astro solo expone al cliente las variables prefijadas con PUBLIC_ (a diferencia de Vite puro,
// que expone todo lo prefijado con VITE_). Por eso este servicio lee PUBLIC_API_URL en vez de
// VITE_API_URL; ver astro.config.mjs / Dockerfile para cómo se inyecta en build time.
const API_URL =
...
export const ticketService = {
  ...
  setSatisfaction: (id: string, score: number) =>
    http<Ticket>(`/api/tickets/${id}/satisfaction`, { method: 'POST', body: JSON.stringify({ score }) }),
  escalate: (id: string) => http<Ticket>(`/api/tickets/${id}/escalate`, { method: 'POST' }),
